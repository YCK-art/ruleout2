...
def is_heading(text: str, font_size: float, is_bold: bool, base_font_size: float = 8.0) -> Tuple[bool, int]:
    ...
    patterns = [
        ...
        (r'^Chapter\s*[IVX]+', 1),  # Chapter I, Chapter II
        (r'^[IVX]+\.\s+', 1),  # I., II., III.
        (r'^\d+\.\s+[가-힣A-Z]', 2),  # 1. 서론
        (r'^\d+\.\d+\s+', 3),  # 1.1
        (r'^\d+\.\d+\.\d+\s+', 4),  # 1.1.1
...
def main():
    ...
    """
    print("=" * 80)
    print("🏥 새로운 4개 PDF 처리 및 Pinecone 업로드")
    print("=" * 80)
    ...
    pdf_files = [guidelines_dir / name for name in new_pdf_names if (guidelines_dir / name).exists()]

    if not pdf_files:
        print(f"❌ 처리할 PDF 파일이 없습니다.")
        sys.exit(1)

    print(f"\n📁 {len(pdf_files)}개의 새로운 PDF 파일 발견")
    for pdf in pdf_files:
        print(f"   - {pdf.name}")

    # Pinecone 인덱스 연결
    print(f"\n🔧 Pinecone 인덱스 '{INDEX_NAME}' 연결 중...")
    try:
        index = pc.Index(INDEX_NAME)
        # 현재 인덱스 통계 확인
        stats = index.describe_index_stats()
        print(f"✅ 인덱스 연결 완료")
        print(f"   현재 벡터 수: {stats.total_vector_count:,}개")
    except Exception as e:
    ...
    for pdf_file in pdf_files:
        try:
            ...
            import traceback
            traceback.print_exc()
            stats_list.append({"filename": pdf_file.name, "sections": 0, "chunks": 0})
    ...
    print("\n" + "=" * 80)
    print(f"✅ 전체 완료!")
    print(f"   처리한 파일 수: {len(stats_list)}")
    ...
    print(f"   전체 벡터 수: {final_stats.total_vector_count:,}개")
    print("=" * 80)
